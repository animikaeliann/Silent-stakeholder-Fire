...
**5 gaps shipped**, ranked strongest evidence first. **3 candidates investigated and rejected** via falsification (see log below) rather than silently dropped.
...
## #4 — When a user downloads or saves media (photos, videos, profile/share images) from Bluesky, it always saves to the phone's general camera roll/DCIM folder, mixing app-saved media in with the user's own camera photos, with no option to choose or set a dedicated save location.

**Verdict:** `UNDER-PRIORITIZED`  |  **Confidence:** `0.7`

> 24 independent reviews (2023-07-25 to 2025-04-15) describe this need. #9868: no milestone yet (recently opened); #10981: no milestone yet (recently opened) Provenance: surfaced via scripts/16_latent_signal_mining.py's Detector 1 (workaround language) casting a broader net than the original 4-gap keyword clustering -- NOT itself a latent/second-order finding. Every one of the 24 supporting reviews states this need directly and plainly (e.g. review-play-05626: 'Please for the love of God change the folder where download images go'). See output/latent_candidates.md for the actual (unshipped) latent near-miss this should not be confused with.

**Confidence math:**

```
corroboration=1.0 (n=24 distinct reviews / 15 cap) * 0.35 = 0.35; signal_count=0.24 (n=24 / 100 cap) * 0.25 = 0.06; latency=1.0 (span=630d / 365 cap) * 0.20 = 0.2; roadmap_disconfirmation=0.45 * 0.20 = 0.09. Sum=0.7 -> rounded to nearest 0.05 = 0.7. Disconfirmation basis: #9868: no milestone yet (recently opened); #10981: no milestone yet (recently opened).
```

**Evidence:**

- *[primary]* (`review-play-01149`): Pretty good so far, but my biggest gripe is where photos are saved. Rather than downloading to a Bluesky folder or even the downloads folder, it goes straight into camera roll, or DCIM. I shouldn't have to manually separate my downloaded pictures from my camera photos, literally
- *[corroborating]* (`review-play-04987`): Sadly downloads images to the DCIM folder instead of the generic Pictures folders meant for apps.
- *[corroborating]* ([github-issue-9868](https://github.com/bluesky-social/social-app/issues/9868)): [Android] Setting to download images / files to a custom (Downloads) folder  ### Describe the Feature  To be brief, I don't consider it normal for my image gallery app to be full of folders generated by random separate apps. I have 4 web browsers, I save all to Downloads. Similar
- *[corroborating]* ([github-issue-10981](https://github.com/bluesky-social/social-app/issues/10981)): Bluesky profile QR code saves to camera folder, not Bluesky folder  ### Steps to Reproduce  1. Open your profile on Bluesky. 2. Tap to share the profile. 3. Save the image.  ### Attachments  _No response_  ### What platform(s) does this occur on?  Android  ### Device Info  _No re

**Roadmap cross-references:**

- [github-issue-9868](https://github.com/bluesky-social/social-app/issues/9868) — exact match: '[Android] Setting to download images / files to a custom (Downloads) folder'
- [github-issue-10981](https://github.com/bluesky-social/social-app/issues/10981) — adjacent-narrower: same underlying save-location bug, but scoped specifically to the profile QR-code share flow, not general photo/media downloads

**Alternative explanations considered and rejected:** Could this be device/OS fragmentation (some Android skins or gallery apps behaving differently) rather than an app-level gap? Ruled out: 24 reviews spanning 630 days consistently describe the SAME underlying behavior (Bluesky hardcodes saved media to the OS default camera-roll/DCIM path regardless of device), using varied phrasing (camera roll, DCIM, gallery, Pictures folder) for the identical complaint, and two separately-filed GitHub issues (#9868, #10981) describe the same root cause from different angles -- not one device's own quirk. Could this be the same need as a distinct 'can't select upload source folder' complaint also found in this corpus (e.g. review-play-00931, review-play-00888)? Ruled out and deliberately excluded: that's the opposite direction (choosing which existing photo to attach when composing, not where a downloaded/saved file ends up) -- bundling them would combine two distinct asks into one gap, the same construction weakness already flagged for gap #3 (no-private-account-remove-follower) during an earlier adversarial-verification pass.

---

## #5 — Follower counts and follower lists don't reflect reality: blocked accounts still count as followers and still appear in follower lists, and displayed follower counts are simply inconsistent/wrong on refresh.
